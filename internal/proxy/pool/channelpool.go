...
// there is a channel to contain Proxy object,
// and provide Get and Put method to handle with RevsereProxy
type chanPool struct {
	// mutex makes the chanPool woking with goroutine safely
	mutex sync.RWMutex

	// reverseProxyChan chan of getting the *Proxy and putting it back
	reverseProxyChan chan *proxy.Proxy

	// factory is factory method to generate Proxy
...
func NewChanPool(initialCap, maxCap int, factory Factory) (Pool, error) {
	...
	pool := &chanPool{
		mutex:            sync.RWMutex{},
		reverseProxyChan: make(chan *proxy.Proxy, maxCap),
		factory:          factory,
	}
...
func (p *chanPool) getConnsAndFactory() (chan *proxy.Proxy, Factory) {
	p.mutex.RLock()
...
// Get a *Proxy from pool, it will get an error while
// reverseProxyChan is nil or pool has been closed
func (p *chanPool) Get(addr string) (*proxy.Proxy, error) {
...
// Put ... put a *Proxy object back into chanPool
func (p *chanPool) Put(proxy *proxy.Proxy) error {
	if proxy == nil {
