...
func TestGenericStore_Init(t *testing.T) {
	tests := []struct {
		...
		giveListErr     error
		giveListRet     []storage2.Keypair
		giveWatchCh     chan storage2.WatchResponse
		giveResp        storage2.WatchResponse
		wantErr         error
	...
	}{
		{
			...
			giveWatchCh: make(chan storage2.WatchResponse),
			giveResp: storage2.WatchResponse{
				Events: []storage2.Event{
					{
						Keypair: storage2.Keypair{
							Key:   "test/demo3-f1",
							Value: `{"Field1":"demo3-f1", "Field2":"demo3-f2"}`,
						},
						Type: storage2.EventTypePut,
					},
					{
						Type: storage2.EventTypeDelete,
						Keypair: storage2.Keypair{
							Key: "test/demo1-f1",
						},
					},
			...
			wantCache: map[string]interface{}{
				"demo2-f1": &TestStruct{
					BaseInfo: entity2.BaseInfo{ID: "demo2-f1"},
					Field1:   "demo2-f1",
					Field2:   "demo2-f2",
				},
				"demo3-f1": &TestStruct{
					BaseInfo: entity2.BaseInfo{ID: "demo3-f1"},
					Field1:   "demo3-f1",
					Field2:   "demo3-f2",
				},
			},
	...
	for _, tc := range tests {
		...
		mStorage := &storage2.MockInterface{}
		...
		mStorage.On("Watch", mock.Anything, mock.Anything).Run(func(args mock.Arguments) {
			watchCalled = true
			assert.Equal(t, tc.giveStore.opt.BasePath, args[1], tc.caseDesc)
		}).Return((<-chan storage2.WatchResponse)(tc.giveWatchCh))
...
func TestGenericStore_StringToObjPtr(t *testing.T) {
	s, err := NewGenericStore(GenericStoreOption{
		BasePath: "test",
		ObjType:  reflect.TypeOf(entity2.SSL{}),
		KeyFunc: func(obj interface{}) string {
			r := obj.(*entity2.Route)
	...
	ssl := sslInterface.(*entity2.SSL)
	assert.Equal(t, id, ssl.ID)
}

type ListInputParams struct {
...
func TestNewListOutput(t *testing.T) {
	err := storage2.InitETCDClient(&storage2.EtcdConfig{
	...
	store, err := NewGenericStore(GenericStoreOption{
		BasePath: "/apisix/routes",
		ObjType:  reflect.TypeOf(entity2.Route{}),
		KeyFunc: func(obj interface{}) string {
			r := obj.(*entity2.Route)
			return convutil.ToString(r.ID)
		},
	...
	listOutput, err := store.List(context.Background(), ListInput{
		Predicate: func(obj interface{}) bool {
			routeObj := obj.(*entity2.Route)
			if input.Name != "" && !strings.Contains(routeObj.Name, input.Name) {
				return false
			}

			if input.URI != "" && !uriContains(routeObj, input.URI) {
				return false
			}
			if input.URI != "" && !reg_uri.KeyMatch4(input.URI, routeObj.URI) {
				return false
			}

			if input.Status != "" && strconv.Itoa(int(routeObj.Status)) != input.Status {
				return false
			}
		...
		Format: func(obj interface{}) interface{} {
			route := obj.(*entity2.Route)
			if route.Upstream != nil && route.Upstream.Nodes != nil {
				route.Upstream.Nodes = entity2.NodesFormat(route.Upstream.Nodes)
			}
			return route
		},
...
func uriContains(obj *entity2.Route, uri string) bool {
