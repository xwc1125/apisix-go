...
var (
	schema              gjson.Result
	rootPath            = "/Users/yijiaren/Workspaces/git/xwc1125/gateway/apisix-go/conf"
	apisixSchemaPath    = path.Join(rootPath, "schema.json")
	customizeSchemaPath = path.Join(rootPath, "customize_schema.json")
)
...
func TestAPISIXJsonSchemaValidator_Validate(t *testing.T) {
	validator, err := NewAPISIXJsonSchemaValidator(schema, "main.consumer")
	assert.Nil(t, err)
...
func TestAPISIXJsonSchemaValidator_checkUpstream(t *testing.T) {
	validator, err := NewAPISIXJsonSchemaValidator(schema, "main.route")
	assert.Nil(t, err)
...
func TestAPISIXSchemaValidator_SystemConfig(t *testing.T) {
	...
	for _, tc := range tests {
		validator, err := NewAPISIXSchemaValidator(schema, tc.givePath)
		if tc.wantNewErr {
			assert.Error(t, err)
...
func TestAPISIXSchemaValidator_Validate(t *testing.T) {
	validator, err := NewAPISIXSchemaValidator(schema, "main.consumer")
	assert.Nil(t, err)
...
type IpRestrictionConf struct {
	...
	Message   string   `json:"message,omitempty"`   // 不允许访问IP地址时返回的消息[1, 1024]
...
`
	validator, err := NewSchemaValidator(schemaStr)
	if err != nil {
		t.Fatal(err)
	}
